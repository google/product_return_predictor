...
_NON_FEATURES = ['transaction_id', 'customer_email_hash',  'refund_product_quantity', 'predefined_split_column', 'partial_refund_boolean', 'refund_product_quantity', 'refund_amt', 'refund_boolean', 'refund_rate', 'transaction_date']
...
def load_predictions_to_table(bigquery_client: bigquery.Client,
  ...
  SELECT
    transaction_id,
    IF(predicted_value.value < 0,
       0,
       predicted_value.value) AS predicted_value
  FROM `{output_table}`
  """
